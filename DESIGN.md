...
- **Sign in (`.nav-signin`):** the way into the application at app.sellercopilot.ai, closing the site nav on every page. An outlined capsule — 40px tall, `0 20px`, 1.5px ink border, `border-radius: 999px`, ink text, filling to ink-on-paper on hover. **It is the one rounded object in a square world and a deliberate exception to the corner rule**, requested by the user so the entry to the product reads as leaving the marketing site; it stays outlined so it never competes with a filled stamp. In the phone menu it sits inside the list as a left-aligned capsule with a 24px side margin, not a full-width row, and carries no row hairline. Needs the `.main-nav a.nav-signin` selector: a plain class loses to `.main-nav a:not(.stamp)`.
