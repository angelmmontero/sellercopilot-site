...
### Hierarchy
- **Display** (600, `clamp(2rem, 3.2vw, 2.7rem)`, 48px line at ≥901px and 1.06 below, wide 110%, −0.035em, max 19ch; 16ch below 900px): the hero h1 only. Its final clause is red via `em` with italics removed.
- **Headline** (600, `clamp(1.7rem, 2.6vw, 2.25rem)`, 48px line at ≥901px / 1.1 below, wide 110%, −0.035em, max 24ch): section h2s and the contact h2. The legal page h1 is the same voice at `clamp(2rem, 3.4vw, 2.6rem)`.
- **Figure** (700, 6rem, 0.86, condensed 68%, −0.03em, red, `white-space: nowrap`, `10px 0 14px` padding): the net-profit reading in the hero strip, counted up by script from `data-count`. Below 900px it becomes `clamp(3.6rem, 15vw, 5.5rem)`. It is deliberately larger than the display and is the one text size not on the ruling.
...
- **Body** (400, 1.0625rem / 1rem below 900px, 24px line, normal width): lead paragraph 1.125rem `ink-soft` max 32rem; section intro 1.1rem `ink-soft` max 40rem; row paragraphs 0.95–1rem `ink-2` at 46–52ch (round 0.98rem, measure 1rem, callout and spec 0.95rem, never-row 0.98rem ink, step 0.98rem, control 0.96rem); legal body 1rem at 68ch with one blank rule between paragraphs. All on the 24px line.
- **Note** (400, 0.85rem, 24px line, `ink-soft`, normal width): the `.note` voice for prose captions, sub-lines and footnotes: the section head's margin note, the chart caption, the rounds note, the measure unit line, the quote and talk notes, the legal "last updated", and the footer. The brand tagline is the same idea at 0.72rem on a 1.2 line. On the ink section `.note` is `paper-dim`.
- **Label** (600, 0.75rem, 24px line, 0.12em, condensed 78%, uppercase, `ink-soft`): the `.label` field-label voice; on this page it sits once, above the figure. In the timeline SVG the same voice is 10.5px with 1.2px tracking; in the plot it is 13px with 1.4px tracking (`.t-label` in `green-ink`, `.t-unit` in `ink-soft`).
- **Action** (600, 0.86rem, 0.08em, condensed 85%, uppercase, `line-height: 1` inside a 48px stamp): stamps. The header stamp drops to 0.78rem in a 40px stamp (0.74rem below 900px); nav links and hero facts are the same voice at 0.8rem with 0.08em / 0.06em tracking, the facts on a 24px line.
...
- **Eyebrow** (Fragment Mono 400, 0.7rem, 24px line, 0.1em, uppercase, `ink-soft`): the `.eyebrow` voice for short mono captions: the receipt title ("Receipt · 14 days"), the callout heading, the "Included" heading over a spec list. The shot caption is the same voice at 0.06em with its right span in `ink-faint`.
- **Reading** (Fragment Mono 400, 0.8rem, 24px line, tracking 0): receipt keys and values, the gain line; the sticker's first row at 0.78rem; the shot placeholder at 0.78rem with 0.02em; plot axis text at 15px in the 720-unit viewBox (about 11px effective at 1440), the call-out value at condensed 700 27px and its sub-line in mono.
...
**The Mono Is A Reading Rule.** Fragment Mono is restricted to `.receipt`, the sticker's identifier and marketplace, the plot's lettering, the figure's gain line, `.shot-caption`, `.shot-placeholder` and `.eyebrow`. A caption, footnote, dateline or sub-line that is prose is set in the Archivo `.note` voice, never in the mono. The test: if it could be printed by the instrument, it is mono; if a person wrote it, it is not.
...
**The Line Is 24px Rule.** Text is set on `line-height: var(--u)` regardless of size: 17px body, 15–16px row copy, 13.6px notes, 12.8px mono and 12px labels all occupy one 24px rule; h2s and the contact email occupy two. A new size picks a line of 24px or 48px, not a ratio. The figure (6rem × 0.86) is the recorded exception.

**Recorded inconsistencies (type).** The size scale is still a spray: condensed caps at 0.72 / 0.75 / 0.78 / 0.8 / 0.86rem with three widths (78 / 85 / 92%) and five trackings (0.06–0.16em); mono at 0.7 / 0.78 / 0.8rem plus 15 / 13 / 27px in the plot and 10.5px in the timeline; small prose at 0.72 (brand tagline) and 0.85rem (`.note`); row prose at 0.95 / 0.96 / 0.98 / 1 / 1.0625 / 1.1 / 1.125rem; titles at 1.1 / 1.2 / 1.35 / 1.5rem. These are recorded as they are; a new element reuses the nearest existing pairing rather than adding another.
...
- **Primary (`.stamp`):** ink fill, paper text, `0 22px` padding, Action voice (600, 0.86rem, 85%, 0.08em, uppercase). In the header: 40px tall, `0 16px`, 0.78rem; `0 14px`, 0.74rem below 900px.
...
- **Brand:** 30px chart-glyph SVG (white square, 20% ruling, 1.5 ink frame, 2.4 ink line, red 3.2r dot) with the wordmark (700, 1.05rem, 92%) over a sans tagline (0.72rem, 1.2 line, `ink-soft`, hidden below 900px).
...
- **Placeholder (`.shot-placeholder`):** absolutely filling the frame, centred, 1u padding, mono 0.78rem 0.02em `ink-soft`: `[ SCREENSHOT: hero-app · 16:10 ]`, `aria-hidden`.
...
- Each `.callout` has a 1px `rule-major` top rule painted as `inset 0 1px 0`, 1u padding above, then an Eyebrow (`.eyebrow`, mono 0.7rem caps) on one 24px line and a 0.95rem `ink-2` paragraph on the 24px line with 1u padding below; `strong` is ink 600.
...
- Each `li` is 0.95rem `ink-2` on a 24px line with no padding and a painted 4px × 1px `rule-major` hairline at its foot that the last item drops; a wrapping item takes two rules and keeps the hairline under its last line.
...
- White fill, 1px ink border, 2px radius, `11px 12px` padding, Stuck-on shadow, mono 0.78rem on a 24px line; exactly 72px tall (three rules). Settled: border and text `ink-soft`, no shadow. Below 520px it stacks to one column and becomes 96px (four rules). In the strip it sits 24px above the plot; in a round it is flush with the row's top padding.
...
- **Reading (`.reading`):** the strip's `figcaption`. `.label` spanning the grid on a 24px line; the `.figure` (6rem red condensed, `data-count="398" data-prefix="−$"`) and `.reading-note` (0.95rem on a 24px line, `ink-2`, max 30rem, strong in ink) side by side aligned at the end; `.gain` is a mono 0.8rem `green-ink` block inside the note; `.chart-caption` is a `.note` spanning the grid 1u below. Recorded: the figure's `10px 0 14px` padding and 0.86 line keep this column off the pitch (see Layout).
...
- **Body (`.round-body`):** a paper plate; h3 Title 1.2rem on a 24px line with one blank rule below; paragraph 0.98rem `ink-2` on a 24px line at 52ch. The first round's paragraph carries `data-typed` and types itself once (see Motion); while typing, a 2px ink caret follows the text (`.is-typing .typed::after`).
...
- `.measures` has a 1px ink top rule (`inset 0 1px 0`); each `.measure` carries `.ruling`, is `200px 1.05fr 1fr` with `2u 0 1u` padding and a painted `rule-major` bottom rule. Column one is the unit line: a `.note` (Archivo 0.85rem `ink-soft`, 24px line) with one `span` per line, unplated. Column two (`.measure-body`, plated): the h3 (1.35rem, 24px line, one blank rule below) and a 1rem `ink-2` paragraph at 46ch, with a 1u margin below when something follows. Column three is either `.measure-list` (plated: "Included" eyebrow + spec list) or, on a `.measure-shot` row, a 4:3 `.shot`, in which case the body also holds `.callouts` 1u under its paragraph.
...
- `.section-head`: h2 and a right-aligned `.note` margin note on one baseline, `24px − 2px` padding below, 2px ink rule, 2u after. `.section-intro` (1.1rem `ink-soft`, 40rem, 24px line) pulls up 1u to sit under the rule and leaves 2u after. Below 900px the meta drops under the h2, left-aligned, and wraps.

### Footer (the form's foot)
- 2px ink top rule, `24px − 2px` padding above and 2u below; a flex row, wrapping, of `.note`-sized (0.85rem, 24px line) `ink-soft` text and links (no underline; red on hover), links spaced 1u.

### Legal pages
- `.legal` is a 32u column padded 3u / 1u / 4u (2u / 1u / 3u below 900px). `.legal-head` mirrors the section head (h1 Headline voice on a 48px line, "last updated" as a 0.85rem `ink-soft` note right, `24px − 2px` padding, 2px ink rule, 2u after). h2s are 1.35rem on a 24px line with a 1px `rule-major` top rule, 1u margin above and below and `24px − 1px` of padding above; body 1rem `ink-2` on a 24px line at 68ch with one blank rule (24px) between paragraphs and after lists; list markers `ink-faint`; links underlined in `rule-major`, red on hover; a stamp 1u below the text where present. 404.html reuses the shell with "Error 404" as the dateline.
...
- **Do** set prose captions, footnotes, datelines and sub-lines in the Archivo `.note` voice (0.85rem, `ink-soft`, 24px line); keep the mono for what an instrument would print.
